...
## Corrections :
## 04/05/2021 : meilleur respect PEP 8 + qques bugs ...
...
class Arbre:
    ...
    def __init__(self, first, **kwargs):#label, listefils=None) :
        ...
        L'étiquette d'un nœud ne DOIT PAS être None
        """
    ...
    def remplace(self, pos, a):
        ...
        if pos<0:
            raise IndexError('Position {} incorrecte'.format(pos))
    ...
    def __getitem__(self, i):
        ...
        Lève IndexError si l'indice est incorrect
        """
        if i < 0:
            i = len(self._fils) - i
        if i not in range(len(self._fils)):
            raise IndexError('Pas de fils {}'.format(i))
        return self._fils[i]
    ...
    def __str__(self):
        ...
        s = ",".join(s)
        if s:
            return "({}-({}))".format(self.__label, s) #"("+str(self.__label)+"-("+s+"))"
        else:
            return str(self.__label)

    def __len__(self):
        """ Renvoie le nombre de fils (différents de None) """
        return len([f for f in self._fils if f is not None])
...
class ArbreBinaire(Arbre):
    ...
    def fg(self):
        "Le fils gauche"
        return self._fils[0] if self._fils else None
    ...
    def fd(self):
        "Le fils droit"
        return self._fils[1] if self._fils else None
